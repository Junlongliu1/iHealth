...
enum BasalEnergyRange: String, CaseIterable, Identifiable {
    case day, week, month
...
struct BasalEnergyDetailView: View {
    ...
    @State private var selectedRange: BasalEnergyRange = .day
    ...
    private var rangePicker: some View {
        Picker("范围", selection: $selectedRange) {
            ForEach(BasalEnergyRange.allCases) { range in
    ...
    private func trendContent(for range: BasalEnergyRange) -> some View {
    ...
    private func fetchWeek(_ anchor: Date) async -> [DailyBasalEnergy] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyBasalEnergy(from: weekStart, to: end)
    }

    private func fetchMonth(_ anchor: Date) async -> [DailyBasalEnergy] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyBasalEnergy(from: monthStart, to: end)
    }
...
private struct BasalEnergyHourlyChart: View {
    ...
    private func bubble(_ item: HourlyBasalEnergy) -> some View {
        HStack(spacing: 5) {
...
private struct BasalEnergyTrendChart: View {
    ...
    private func bubble(_ item: DailyBasalEnergy) -> some View {
        HStack(spacing: 5) {
...
private struct BasalEnergyStatsGrid: View {
    ...
    private var best: DailyBasalEnergy? {
        daily.max { $0.kilocalories < $1.kilocalories }
    }

    private var lowest: DailyBasalEnergy? {
