...
enum ActiveEnergyRange: String, CaseIterable, Identifiable {
    case day, week, month
...
struct ActiveEnergyDetailView: View {
    ...
    @State private var selectedRange: ActiveEnergyRange = .day
    ...
    private var rangePicker: some View {
        Picker("范围", selection: $selectedRange) {
            ForEach(ActiveEnergyRange.allCases) { range in
    ...
    private func trendContent(for range: ActiveEnergyRange) -> some View {
    ...
    private func fetchWeek(_ anchor: Date) async -> [DailyActiveEnergy] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyActiveEnergy(from: weekStart, to: end)
    }

    private func fetchMonth(_ anchor: Date) async -> [DailyActiveEnergy] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyActiveEnergy(from: monthStart, to: end)
    }
...
private struct ActiveEnergyHourlyChart: View {
    ...
    private func bubble(_ item: HourlyActiveEnergy) -> some View {
        HStack(spacing: 5) {
...
private struct ActiveEnergyTrendChart: View {
    ...
    private func bubble(_ item: DailyActiveEnergy) -> some View {
        HStack(spacing: 5) {
...
private struct ActiveEnergyStatsGrid: View {
    ...
    private var best: DailyActiveEnergy? {
        daily.max { $0.kilocalories < $1.kilocalories }
    }

    private var lowest: DailyActiveEnergy? {
        daily.min { $0.kilocalories < $1.kilocalories }
    }
