...
enum DaylightRange: String, CaseIterable, Identifiable {
    case day, week, month
...
struct DaylightDetailView: View {
    ...
    @State private var selectedRange: DaylightRange = .day
    ...
    private var rangePicker: some View {
        Picker("范围", selection: $selectedRange) {
            ForEach(DaylightRange.allCases) { range in
    ...
    private func trendContent(for range: DaylightRange) -> some View {
    ...
    private func fetchWeek(_ anchor: Date) async -> [DailyDaylight] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyDaylight(from: weekStart, to: end)
    }

    private func fetchMonth(_ anchor: Date) async -> [DailyDaylight] {
        let calendar = Calendar.current
        ...
        return await healthManager.fetchDailyDaylight(from: monthStart, to: end)
    }
...
private struct DaylightHourlyChart: View {
    ...
    private func bubble(_ item: HourlyDaylight) -> some View {
        HStack(spacing: 5) {
...
private struct DaylightTrendChart: View {
    ...
    private func bubble(_ item: DailyDaylight) -> some View {
        HStack(spacing: 5) {
...
private struct DaylightStatsGrid: View {
    ...
    private var best: DailyDaylight? {
        daily.max { $0.minutes < $1.minutes }
    }

    /// 达到 30 分钟以上的天数（常见日照建议值）
    private var reachedDays: Int {
        daily.filter { $0.minutes >= 30 }.count
    }
