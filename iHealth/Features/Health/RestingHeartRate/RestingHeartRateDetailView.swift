...
struct RestingHeartRateDetailView: View {
    ...
    private var dayContent: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                Text("静息心率").font(.system(size: 13)).foregroundStyle(.secondary)
                HStack(alignment: .firstTextBaseline, spacing: 4) {
                    Text(hasAny ? "\(Int(avg.rounded()))" : "--")
                        .font(.system(size: 28, weight: .bold, design: .rounded))
                        .foregroundStyle(.red).monospacedDigit()
                        .lineLimit(1).minimumScaleFactor(0.6)
                    Text("次/分").font(.system(size: 13)).foregroundStyle(.secondary)
                }
            }
    ...
    private func trendContent(for range: MetricRange) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                Text("日均静息心率").font(.system(size: 13)).foregroundStyle(.secondary)
                HStack(alignment: .firstTextBaseline, spacing: 4) {
            ...
            LazyVGrid(columns: [
                ...
                statItem(title: "最高",     value: values.max().map { "\(Int($0.rounded())) 次/分" } ?? "--")
                statItem(title: "最低",     value: values.min().map { "\(Int($0.rounded())) 次/分" } ?? "--")
    ...
    private func shift(by offset: Int) {
        ...
        case .week(let newWeek):
            Task {
                ...
                let fetched = await healthManager.fetchDailyRestingHeartRate(from: s, to: e)
                withAnimation(.smooth(duration: 0.32)) {
    ...
    private func loadCurrent() async {
        ...
        case .week:
            ...
            daily = await healthManager.fetchDailyRestingHeartRate(from: s, to: e)
        case .month:
