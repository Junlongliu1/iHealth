...
enum RestingHeartRateRange: String, CaseIterable, Identifiable {
    case day, week, month
...
struct RestingHeartRateDetailView: View {
    ...
    @State private var selectedRange: RestingHeartRateRange = .day
    @State private var currentDay: Date = Calendar.current.startOfDay(for: Date())
    @State private var weekAnchor: Date = Date()
    @State private var monthAnchor: Date = Date()

    @State private var hourly: [HourlyRestingHeartRate] = []
    @State private var daily: [DailyRestingHeartRate] = []
    ...
    private var dayContent: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if hasAny {
                    RestingHeartRateHourlyChart(data: hourly, selection: $selectedHour)
                        .frame(height: 140)
                } else {
    ...
    private func trendContent(for range: RestingHeartRateRange) -> some View {
        let avg = averageDaily(daily)
        let values = daily.compactMap { $0.bpm }
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(
                ...
                statItem(title: "平均",     value: avg.map { "\(Int($0.rounded())) 次/分" } ?? "--")
                statItem(title: "最高",     value: highest.map { "\(Int($0.rounded())) 次/分" } ?? "--")
                statItem(title: "最低",     value: lowest.map  { "\(Int($0.rounded())) 次/分" } ?? "--")
    ...
    private func shift(by offset: Int) {
        ...
        case .day:
            ...
            selectedHour = nil
            Task {
                let fetched = await healthManager.fetchHourlyRestingHeartRate(for: newDay)
    ...
    private func loadCurrent() async {
        ...
        switch selectedRange {
        case .day:
            hourly = await healthManager.fetchHourlyRestingHeartRate(for: currentDay)
        case .week:
    ...
    private func fetchWeek(_ anchor: Date) async -> [DailyRestingHeartRate] {
        let calendar = Calendar.current
        ...
        let end = isCurrentWeek
            ...
            : weekEnd
        return await healthManager.fetchDailyRestingHeartRate(from: weekStart, to: end)
    }

    private func fetchMonth(_ anchor: Date) async -> [DailyRestingHeartRate] {
        let calendar = Calendar.current
        ...
        let end = isCurrentMonth
            ? calendar.date(byAdding: .day, value: 1, to: calendar.startOfDay(for: now))!
            : interval.end
        return await healthManager.fetchDailyRestingHeartRate(from: monthStart, to: end)
    }

    private func averageDaily(_ data: [DailyRestingHeartRate]) -> Double? {
...
private struct RestingHeartRateHourlyChart: View {
    ...
    private func bubble(_ item: HourlyRestingHeartRate) -> some View {
        HStack(spacing: 5) {
...
private struct RestingHeartRateTrendChart: View {
    ...
    private func bubble(_ item: DailyRestingHeartRate) -> some View {
        HStack(spacing: 5) {
